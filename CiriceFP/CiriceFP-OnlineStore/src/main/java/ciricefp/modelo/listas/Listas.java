...
public class Listas<T> implements Iterable<T> {
    ...
    @SafeVarargs
    public final boolean add(@NotNull T... elementos) {

        // Convertimos el array de elementos en una lista para poder usar el método forEach()
        ArrayList<T> lista = new ArrayList<>(Arrays.asList(elementos));

        try {
            // Recorremos la lista de elementos y los añadimos a la lista mediante una función Lambda.
            lista.forEach(elemento -> this.lista.add(elemento));
